  ...
  return (
    <div className="mx-auto flex min-h-full w-full max-w-[1500px] flex-1 flex-col px-3 lg:px-0">
      ...
      <section className="flex flex-wrap gap-2 pt-1">
        <button type="button" onClick={() => setActiveView("ats")} className={`inline-flex h-12 items-center gap-2 rounded-t-xl rounded-b-md border px-5 text-sm font-black shadow-sm transition ${activeView === "ats" ? "border-slate-200 border-b-[#0f66ff] bg-white text-[#005cff] shadow-[0_14px_30px_rgba(15,23,42,0.08)]" : "border-transparent bg-transparent text-[#344a78] hover:bg-white/70"}`}>
          <FileSpreadsheet size={18} />
          Procesar ATS
        </button>
        <button type="button" onClick={() => setActiveView("diario")} className={`inline-flex h-12 items-center gap-2 rounded-t-xl rounded-b-md border px-5 text-sm font-black shadow-sm transition ${activeView === "diario" ? "border-slate-200 border-b-[#0f66ff] bg-white text-[#005cff] shadow-[0_14px_30px_rgba(15,23,42,0.08)]" : "border-transparent bg-transparent text-[#344a78] hover:bg-white/70"}`}>
          <FileSpreadsheet size={18} />
          Libro Diario
        </button>
        <button type="button" onClick={() => setActiveView("mayor")} className={`inline-flex h-12 items-center gap-2 rounded-t-xl rounded-b-md border px-5 text-sm font-black shadow-sm transition ${activeView === "mayor" ? "border-slate-200 border-b-[#0f66ff] bg-white text-[#005cff] shadow-[0_14px_30px_rgba(15,23,42,0.08)]" : "border-transparent bg-transparent text-[#344a78] hover:bg-white/70"}`}>
          <FileSpreadsheet size={18} />
          Libro Mayor
        </button>
        <button type="button" onClick={() => setActiveView("balance")} className={`inline-flex h-12 items-center gap-2 rounded-t-xl rounded-b-md border px-5 text-sm font-black shadow-sm transition ${activeView === "balance" ? "border-slate-200 border-b-[#0f66ff] bg-white text-[#005cff] shadow-[0_14px_30px_rgba(15,23,42,0.08)]" : "border-transparent bg-transparent text-[#344a78] hover:bg-white/70"}`}>
          <FileSpreadsheet size={18} />
          ...
          <section className="space-y-5">
            <section>
              <LibroDiarioTab
                ...
                onExportExcel={() => void downloadJournalExcel(response, rucActivo).catch((err) => setError(err instanceof Error ? err.message : "No se pudo exportar el Libro Diario."))}
              />
            </section>
            <LoteActions
              response={response}
      ...
      {activeView === "ats" && (
      <>
      <section className="rounded-[18px] border border-slate-200 bg-white px-6 py-4 shadow-[0_12px_32px_rgba(15,23,42,0.07)]">
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
          <div className="flex items-center gap-4">
            <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-[16px] bg-[#eaf2ff] text-[#0f66ff]">
              <FileSpreadsheet size={26} />
            </div>
            <div>
              <h1 className="text-[22px] font-black leading-tight text-[#08265f]">Procesos Contables desde ATS</h1>
              <p className="mt-1 text-sm font-semibold text-[#5b6f9c]">
                Solo se leerán las pestañas COMPRAS, VENTAS y GASTOS
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2 rounded-lg border border-[#bcd9ff] bg-[#f2f8ff] px-4 py-2 text-xs text-[#425783]">
            <InfoIcon className="shrink-0 text-[#0f66ff]" size={17} />
            <p className="font-bold leading-snug">
              Pestañas permitidas: <span className="font-black text-[#005cff]">COMPRAS, VENTAS y GASTOS</span>
            </p>
          </div>
        </div>
      </section>

      <section className="rounded-[18px] border border-slate-200 bg-white p-5 shadow-[0_12px_32px_rgba(15,23,42,0.07)]">
        <h2 className="text-base font-black text-[#08265f]">Archivo Excel ATS</h2>
        <input
          ref={fileInputRef}
          type="file"
          accept=".xlsx,.xls"
        ...
        <div
          ...
          className={`mt-3 flex min-h-[238px] cursor-pointer flex-col items-center justify-center rounded-[16px] border border-dashed px-5 py-6 text-center transition ${
            dragOver
              ? "border-[#0f66ff] bg-[#f3f8ff]"
              : "border-[#b7c7df] bg-white hover:border-[#72a7ff] hover:bg-[#fbfdff]"
          }`}
        >
          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#edf3ff] text-[#0f66ff]">
            <UploadCloud size={26} />
          </div>
          <p className="mt-4 text-lg font-black text-[#08265f]">Arrastra y suelta tu archivo ATS aquí</p>
          <p className="mt-2 text-sm font-semibold text-[#62739b]">o selecciona un archivo desde tu dispositivo</p>
          <button
            type="button"
            onClick={(event) => {
            ...
            className="mt-5 inline-flex h-12 min-w-[240px] items-center justify-center gap-3 rounded-lg bg-[#0f66ff] px-6 text-sm font-black text-white shadow-[0_12px_24px_rgba(15,102,255,0.22)] transition hover:bg-[#0057e6] focus:outline-none focus:ring-4 focus:ring-blue-200"
          >
            <FileSpreadsheet size={20} />
            Seleccionar archivo
          </button>
          <p className="mt-4 text-xs font-semibold text-[#62739b]">
            Formatos permitidos: .xlsx, .xls <span className="mx-3 text-slate-400">|</span> Tamaño máximo: 25MB
          </p>
        </div>

        {archivo && (
          <div className="mt-4 flex flex-col gap-3 rounded-xl border border-blue-100 bg-[#f7fbff] p-3 md:flex-row md:items-center md:justify-between">
        ...
        <div className="mt-4 flex items-center gap-3 rounded-lg border border-[#bcd9ff] bg-[#f5fbff] px-4 py-2 text-xs font-bold text-[#005cff]">
          <InfoIcon className="shrink-0" size={17} />
          <p>
...
function ActionButton({
  ...
  const toneClass = {
    primary: "border-[#0f66ff] bg-[#0f66ff] px-6 text-white shadow-[0_14px_26px_rgba(15,102,255,0.26)] hover:bg-[#0057e6] hover:border-[#0057e6]",
    warning: "border-amber-300 bg-amber-50 text-amber-900 shadow-sm hover:bg-amber-100",
    secondary: "border-slate-200 bg-white text-[#506398] shadow-sm hover:border-[#b8c6e4] hover:bg-slate-50",
  }[tone];
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`inline-flex h-11 items-center justify-center gap-2 rounded-xl border px-4 text-sm font-black transition disabled:cursor-not-allowed disabled:opacity-50 ${toneClass}`}
    >
      {icon}
      {label}
    </button>
...
function LoteActions({
  ...
  return (
    <section className="rounded-[18px] border border-slate-200 bg-white p-4 shadow-[0_14px_34px_rgba(15,23,42,0.05)]">
